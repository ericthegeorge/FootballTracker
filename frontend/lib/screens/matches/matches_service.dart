...
class MatchesService {
  ...
  static Future<List<Map<String, dynamic>>> fetchMatchesWithTeams() async {
  final response = await http.get(Uri.parse('$baseUrl/matches-with-teams/'));
  if (response.statusCode == 200) {
    return List<Map<String, dynamic>>.from(jsonDecode(response.body));
  } else {
    throw Exception('Failed to load matches with teams');
  }
}
}
